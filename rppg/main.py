...
def norm_bvp(bvp, sr=30):
    ...
    peaks = np.sort(
        np.concatenate(
            [
                find_peaks(bvp_, prominence=prominence, distance=0.25 * sr)[0],
                find_peaks(-bvp_, prominence=prominence, distance=0.25 * sr)[0],
            ]
        )
    )
    l = [
        ((x - (np.max(x) + np.min(x)) / 2) / (np.max(x) - np.min(x)))
        for x in (bvp_[a:b] for a, b in zip(peaks, peaks[1:] + 1))
    ]
...
def detrend(signal, min_freq=0.5, sr=30):
    ...
    offsets = [0, 1, 2]
    D = diags(
        diags_data, offsets, shape=(signal_length - 2, signal_length), format="csc"
    )
...
class FaceDetector:
    ...
    def _generate_anchors_full(self):
        ...
        while layer_id < self.num_layers:
            ...
            while (
                last_same_stride_layer < self.num_layers
    ...
    def detect(self, image):
        ...
        for idx in selected_indices:
            results.append((boxes_xyxy[idx], keypoints_list[idx], valid_scores[idx]))
...
class Model:
    ...
    def __enter__(self):
        ...
        self.face_detection_semaphore = threading.Semaphore(
            self.face_detection_threads + self.frame_buffer_size
        )
        self.face_detection_pool = ThreadPoolExecutor(
            max_workers=self.face_detection_threads
        )
        self.face_resampling_pool = ThreadPoolExecutor(
            max_workers=self.face_resampling_threads
        )
        ...
        if self.face_mode == "Near":
            self.detector = FaceDetector(
                pkg_resources.resource_filename("rppg", "weights/blaze_face.onnx")
            )
        else:
            self.detector = FaceDetector(
                pkg_resources.resource_filename("rppg", "weights/blaze_face_full.onnx")
            )
        self.alive = True
        ...
        def inference():
            try:
                while self.alive or (len(self.face_buff) >= self.meta["input"][0]):
                    ...
                    face_imgs = list(
                        self.face_resampling_pool.map(
                            lambda x: (x[0](), x[1]), face_imgs
                        )
                    )
                ...
                if len(self.face_buff):
                    face_imgs = self.face_buff + [self.face_buff[-1]] * (
                        self.meta["input"][0] - len(self.face_buff)
                    )
                    face_imgs = list(
                        self.face_resampling_pool.map(
                            lambda x: (x[0](), x[1]), face_imgs
    ...
    def update_frame(self, frame, ts=None):
        ...
        self.face_detection_task = self.face_detection_pool.submit(
            lambda: detect(self.face_detect_count, frame, lock1, lock2, ts, dt)
        )
    ...
    def __update_Kalman(self, frame, ts, box=np.array([]), dt=1 / 30):
        if not isinstance(box, str):
            if len(box):
                ...
                self.box is None
                or ts + 1 - self.hasface > 1
                or (self.rbox[:, 0] <= np.array(0.05) * self.input[1:3]).any()
                or (self.rbox[:, 1] >= np.array(0.95) * self.input[1:3]).any()
            ):
                self.hasface = 0
            if len(box):
                if self.boxkf is None:
                    kbox, self.boxkf = box, [
                        KalmanFilter1D(0.01, 0.5, i, 1) for i in box.reshape(-1)
                    ]
                else:
                    kbox = np.array(
                        [
                            round(k.update(i, dt))
                            for k, i in zip(self.boxkf, box.reshape(-1))
                        ]
                    ).reshape((2, 2))
                self.rbox = box
                if (
                    self.box is None
                    or max(
                        np.abs((np.mean(kbox, axis=1) - np.mean(self.box, axis=1)))
                        / np.mean(self.box, axis=0)
                    )
                    > 0.02
                ):
                    self.box = kbox
            ...
            if self.boxkf is not None:
                kbox = np.array(
                    [
                        round(k.update(i, dt))
                        for k, i in zip(self.boxkf, self.rbox.reshape(-1))
                    ]
                ).reshape((2, 2))
        return self.box
    ...
    def video_capture(self, vid_path=0):
        ...
        self.run = threading.Thread(
            target=lambda: self.__process_video_capture(vid_path, api)
        )
        self.run.start()
        stop = self.stop
    ...
    def process_video_tensor(self, tensor, fps=30.0):
        # tensor shape (length, H, W, RGB) uint8
        if tensor.dtype != "uint8" or len(tensor.shape) != 4 or tensor.shape[-1] != 3:
            raise TypeError(
                "Only processes uint8 tensors with shape (length, H, W, RGB) and values in the range of 0 to 255."
            )
    ...
    def process_video(self, vid_path):
        ...
        if len(tsarr) > 2:
            ...
            if not (self.fps * 0.95 < fps < self.fps * 1.05):
                logger.warning(
                    "Frame rate mismatch, performing nearest neighbor sampling."
                )
                goodvid = False
            if fps_std > 0.05 * fps:
                logger.warning(
                    "Frame rate is unstable, performing nearest neighbor sampling."
                )
                goodvid = False
            if self.statistic["dependent"] > 0:
                logger.warning(
                    "Detected non-key frames, this will damage the rPPG signal. For information on key frames, see https://en.wikipedia.org/wiki/I-frame. \nPlease use https://github.com/KegangWangCCNU/PhysRecorder to record videos that only contain key frames."
                )
                goodvid = False
            if not goodvid:
